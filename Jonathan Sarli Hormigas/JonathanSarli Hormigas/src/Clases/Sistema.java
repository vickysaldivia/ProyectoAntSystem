...
public class Sistema {
    ...
    public void moverHormiga(Hormiga hormiga){
        ...
        for (int i = 0; i < grafo.getSize(); i++){
            if(hormiga.ciudadActual.getNext!= null);
                hormiga.ciudadActual.getCiudadesCandidatas(Hormiga hormiga);
                ciudadesCandidatas.obtenerProbabilidades();
                for(int i = 0; i < grafo.ciudadesCandidatas.getSize(); i++)
                    if(ciudadesCandidatas.get(i)>) //mayor que que la otra);
                        hormiga.visitarCiudad(ciudadInico, i);
                        if (hormiga.getCiudadActual() == ciudadFinal);
                        actualizarFeromonas();
    ...
    public ListaSimple<(double)> obtenerProbabilidades(ListaSimple<Ciudad> ciudadesCandidatas){
        ...
        // [(feromonas entre ciudadInico y ciudadFinal)^alpha(1)*(1/ciudadInico y ciudadFinal)^beta]/la suma de (feromonas entre ciudadInico y ciudadFinal)^alpha(1)*(1/ciudadInico y ciudadFinal)^beta para cada camino
        
        
        ciudadesCadidatas(Hormiga hormiga).setAlfa(double 1);
        ciudadesCadidatas(Hormiga hormiga).setBeta(double 2);
        return null;
    }
